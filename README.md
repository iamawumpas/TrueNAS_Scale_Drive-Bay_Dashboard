...
Check that that the service is actually running.
...
The first entry shows that the service is running, the second shows that that the 'ps aux | grep service.py' has executed. If you only see the second command then the service has failed to run for some reason.
...
*service.py* 
this is the daemon that will interrogate TrueNAS and your HBA to identify the number of used ports, the serial number of each disk (for identification purposes), the slot number on each breakout cable, the disk capacity, the drive status, and read/write activity. It is also a very basic html server which will serve the data to what ever browser you are using to see the dashboard.

*index.html*
this is the HTML (and embedded CSS) needed to view the 
